...
2. **SIS rescue.** When all retries exhaust, the bridge calls `SingleImageSearch` directly at the same coords. SIS hits a different backend than Maps JS and reliably finds the panos `getPanorama` is missing. The parser (`lib/photospheres.js:parseUgcUrlFromResponse`) extracts full metadata (panoid, snapped lat/lng, originHeading, originPitch, copyright, gpms-cs-s render URL) — the bridge synthesizes a normal `PANO_INFO {kind:'ugc', ...}` and the content side renders it transparently via the existing UGC path. End result: 0% no-coverage on routes where coverage genuinely exists.
